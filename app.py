...
glucose = st.sidebar.number_input("Glukosa", min_value=0.0, max_value=300.0, value=120.0)
blood_pressure = st.sidebar.number_input("Tekanan Darah", min_value=0.0, max_value=200.0, value=70.0)
skin_thickness = st.sidebar.number_input("Ketebalan Kulit", min_value=0.0, max_value=100.0, value=20.0)
insulin = st.sidebar.number_input("Insulin", min_value=0.0, max_value=1000.0, value=79.0)
bmi = st.sidebar.number_input("BMI", min_value=0.0, max_value=70.0, value=32.0)
dpf = st.sidebar.number_input("Diabetes Pedigree Function", min_value=0.0, max_value=3.0, value=0.5)
age = st.sidebar.number_input("Usia", min_value=1, max_value=120, value=33)
...
# -------------------------------
if st.sidebar.button("Diagnosa Sekarang"):
    ...
    if prediksi[0] == 1:
        st.error("⚠️ Anda TERDETEKSI Diabetes")
    else:
    ...
    # -------------------------------
    st.subheader("📊 Perbandingan dengan Rata-rata Dataset")
